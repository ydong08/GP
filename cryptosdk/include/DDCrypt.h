...
namespace DDCrypt
{
	...
	//************************************
	// Method:    GetMsgLen							获取消息的实际长度，拼包用
	// FullName:  DDCrypt::GetMsgLen
	// Access:    public 
	// Returns:   int								-1数据不正确，0 还没到包头长度，大于0为包的总长度
	// Qualifier:
	// Parameter: const char * szMsg				消息指针
	// Parameter: const unsigned int nMsgLen		消息当前长度
	//************************************
	int GetMsgLen(const char* szMsg, const unsigned int nMsgLen);
#ifdef __cplusplus
	}
#endif
