...
int IProcess::GameOver(Table* table)
{
	...
	for(i = 0; i < GAME_PLAYER; ++i)
	{
		...
		if(player)
		{
			...
			sendnum++;
            if (!player->isonline && player->source != E_MSG_SOURCE_ROBOT)
            {
	...
	for(i = 0; i < GAME_PLAYER; ++i)
	{
		...
		if(player)
		{
			...
			response.End();
			_LOG_INFO_("[GameOver] GameID[%s] uid[%d] sid[%d] win[%ld] return[%ld] tian[%ld] di[%ld] xuan[%ld] huang[%ld] Money[%ld] tax[%d] reward[%ld]\n", table->getGameID(),player->id, player->m_nSeatID, player->m_lWinScore, 
...
int IProcess::updateDB_UserCoin(Player* player, Table* table)
{
    ...
    outputPacket.WriteInt(player->isonline?GAME_END_NORMAL:GAME_END_DISCONNECT);
	...
	outputPacket.WriteInt(player->m_lRewardCoin);		//taskcoin
	outputPacket.WriteInt(0);							//m_nMagicNum
	outputPacket.WriteInt(0);							//m_nMagicCoinCount
	outputPacket.End();
