...
typedef struct _Cfg
{
	...
	int tax;
	_Cfg():type(0), bankerwincount(0), betwincount(0), limitcoin(100000), retaincoin(10000),
		starthour(0), startmin(0), endhour(0), endmin(0),
		upperlimit(90000000),lowerlimit(-900000),bankerupperlimit(90000000),
		bankerlowerlimit(-900000),jackpot(0),jackpotswitch(50000000), tax(5)
	{}
}Cfg;
...
class Table
{
	...
	public:
		...
		inline void setGameID(const char* id) 
		{
			strncpy(GameID, id,sizeof(GameID));
	...
	public:
		...
		void GameOver();
		void CalculateScore(short resultret = 0);
		...
		void ReceivePushCardType(const std::vector<int>& cardtypes);
	...
	public:
		...
		std::vector<int> m_vecBlacklist;		//黑名单用户
	public:
		//当前桌子是否是在踢人倒计时
	...
	private:
		...
		std::vector<int> receive_push_card_type; //收到php推送的牌型
