...
int ServerManager::addServer(int id,  GameServer* server)
{
	...
	redisReply* reply = RedisConnector->CommandV("HMSET Server:%d  svid %d  "
																					...
																					0, server->lasttime,
																					server->gameID);  //add by maoshuoqiong 20160808
    if(reply)
	{
...
int ServerManager::updateServerMember(int id, GameServer* server)
{
	...
	if(it != _svidsMap.end())
	{
		...
		pserver->gameID = server->gameID;  //add by maoshuoqiong 20160808
	}
	else
...
int ServerManager::getServer(int id,  GameServer* server, bool isredis)
{
	...
	if(!isredis)
	{
		...
		if(it != _svidsMap.end())
		{
			...
			server->gameID = pserver->gameID;  //add by maoshuoqiong 20160808
			return 0;
		}
		else
		{
			...
			else
			{
				for (unsigned int j = 0; j < reply->elements; j++) 
				{
					...
					switch (j)
					{
						...
						case 1:strcpy(server->ip,reply->element[j]->str);break;
