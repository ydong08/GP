...
int GetRoomInfoProcess::doGetRoomInfo(CDLSocketHandler* clientHandler,int PageNo, int PageNum)
{
		...
		for(int i=start; i<end; ++i)
		{
			...
			if(table->player_array[0])
			{
				...
				response.WriteShort(table->player_array[0]->status);
			}
			else
			{
				response.WriteInt(0);
			...
			if(table->player_array[1])
			{
				...
				response.WriteShort(table->player_array[1]->status);
			}
			else
			{
				response.WriteInt(0);
			...
			if(table->player_array[2])
			{
				...
				response.WriteShort(table->player_array[2]->status);
			}
			else
			{
				response.WriteInt(0);
			...
			if(table->player_array[3])
			{
				...
				response.WriteShort(table->player_array[3]->status);
			}
