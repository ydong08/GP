...
LevelMatchProc::LevelMatchProc()
{
	this->name = "LevelMatchProc";
}
...
int LevelMatchProc::doRequest(CDLSocketHandler* client, InputPacket* pPacket, Context* pt )
{
	...
	_LOG_INFO_("==>[LevelMatchProc] [0x%04x] uid[%d]\n", cmd, uid);
	...
	{
		_LOG_DEBUG_("Transfer[%d] LevelMatchProc request to Back_UserServer OK\n" ,uid);
		return 1;
	}
	else
	{
		_LOG_ERROR_("Transfer[%d] LevelMatchProc request to Back_UserServer Error\n",uid );
		return -1;
	}
...
int LevelMatchProc::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt)  
{
	_NOTUSED(clientHandler);
	if(pt==NULL)
	{
       _LOG_ERROR_("[LevelMatchProc:doResponse]Context is NULL\n");	
	   return -1;
	}
	...
	_LOG_INFO_("==>[LevelMatchProc] doResponse [0x%04x]\n", cmd);
	...
	if(pTable == NULL)
	{
		_LOG_ERROR_("[LevelMatchProc] pTable is NULL uid[%d] level[%d]\n", uid, level);
