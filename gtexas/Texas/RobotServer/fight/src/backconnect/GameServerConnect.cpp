...
int GameConnector::connect(const char* ip, short port)
{
	...
	else
	{
		...
		this->ScanningServer();
		short monitortime = Configure::instance()->monitortime;
...
{
//	_LOG_INFO_("Player size[%d] level[%d]\n", PlayerManager::getInstance()->getPlayerSize(),Configure::instance()->level);
	if(this->isActive())
	{
		this->ScanningServer();
	}
	...
	StartTimer(Configure::instance()->monitortime * 1000);
}
