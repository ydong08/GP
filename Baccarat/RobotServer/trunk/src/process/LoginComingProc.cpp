...
LoginComingProc::LoginComingProc() :BaseProcess(false) 
{
...
int LoginComingProc::doRequest(CDLSocketHandler* client, InputPacket* pPacket, Context* pt )
{
	...
	packet.WriteShort((short)E_LEVEL_HUNDRED);
...
int LoginComingProc::doResponse(CDLSocketHandler* client, InputPacket* inputPacket, Context* pt )
{
	...
	if(uid == comingUid)
	{
		...
		if(Configure::getInstance()->level == 2 && tstatus == STATUS_TABLE_BET && limittime > 1)
			player->startBetTimer(1);
		if(Configure::getInstance()->level == 3 && tstatus == STATUS_TABLE_BET)
			player->startBetTimer(1);
