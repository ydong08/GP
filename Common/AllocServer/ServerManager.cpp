...
CDLSocketHandler* ServerManager::getGameServerHandlerByLevel(int gameLevel, int gameID /*= 0*/)
{
	// s1. get game_server_set by gameID
	// ...

	// s2. get unique_game_server by gameLevel
	// s2.1 get svid
	int destSvrID = -1;
	std::map<int, GameServer*>::iterator itor = _svidsMap.begin();
	for (; itor != _svidsMap.end(); ++itor)
	{
		GameServer *pItem = itor->second;
		if (NULL == pItem) continue;

		if (pItem->level == gameLevel)
		{
			destSvrID = pItem->svid;
			break;
		}
	}

	// s2.2 get handler
	return getGameServerHandler(destSvrID);
}
