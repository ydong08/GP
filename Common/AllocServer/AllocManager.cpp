...
{
	if(usercount < 0 || usercount > Configure::getInstance()->numplayer)
	{
...
void AllocManager::delTablesInSvid(short svid)
{
	...
	while( it != _tablelistMap.end())
	{
		...
		if(pTable && pTable->m_nServerId == svid)
		{
			...
			for(i = 0 ; i <= Configure::getInstance()->numplayer; ++i)
			{
...
bool AllocManager::isaccordwithMoney(int64_t currmoney, int64_t money)
{
	...
	if(money > Configure::getInstance()->highlimit)
	{
		if(currmoney > Configure::getInstance()->highlimit)
			return true;
		if(currmoney*Configure::getInstance()->mul > money)
			return true;
		return false;
	}

	if(currmoney/ Configure::getInstance()->mul < money && money < currmoney*Configure::getInstance()->mul)
		return true;
	return false;
}
	...
	while (it != _tablelistMap.end())
	{
		...
		if(pTempTable != NULL)
		{
			...
			if(pServer != NULL)
			{
				...
				if(pServer->status == 1 && pTempTable->m_nTid != tid && pTempTable->m_nUserCount < Configure::getInstance()->numplayer
...
GameTable* AllocManager::getAllocTable(short level, int tid, int64_t money)
{
	GameTable *pTable = NULL;
	int i = 0;
	for(i = Configure::getInstance()->numplayer - 1 ; i >= 0; --i)
	{
...
void AllocManager::getPrivateList(TableListMap &list)
{
	...
	for(; iter!= _tablelistMap.end(); iter++)
	{
		...
		if(pTable != NULL)
		{
			...
			{
				if(pTable->m_nUserCount>0 && pTable->m_nUserCount<= Configure::getInstance()->numplayer)
				{
