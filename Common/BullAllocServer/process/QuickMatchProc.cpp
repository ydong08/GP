...
int QuickMatchProc::doRequest(CDLSocketHandler* client, InputPacket* pPacket, Context* pt )
{
	...
	_LOG_INFO_("==>[QuickMatchProc] [0x%04x] uid[%d]\n", cmd, uid);
	...
	{
		_LOG_DEBUG_("Transfer[%d] QuickMatchProc request to Back_UserServer OK\n" ,uid);
		return 1;
	}
	else
	{
		_LOG_ERROR_("Transfer[%d] QuickMatchProc request to Back_UserServer Error\n",uid );
		return -1;
	}
...
int QuickMatchProc::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt)  
{
	_NOTUSED(clientHandler);
	if(pt==NULL)
	{
       _LOG_ERROR_("[QuickMatchProc:doResponse]Context is NULL\n");	
	   return -1;
	}
	...
	_LOG_INFO_("==>[QuickMatchProc] doResponse [0x%04x] retno=%d \n", cmd, retno);
	...
	if(pTable == NULL)
	{
		_LOG_ERROR_("[QuickMatchProc] pTable is NULL uid[%d] level[%d]\n", uid, level);
