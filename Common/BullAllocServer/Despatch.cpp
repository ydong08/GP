...
int Despatch::doFrontRequest(CDLSocketHandler* clientHandler,  InputPacket* pPacket)
{
	...
	_LOG_DEBUG_( "Recv Front Request, Cmd=[0x%04x] Source[%d] sn[%d]\n" ,nCmdType,Source,sn) ;
	...
	if(processer)
	{
		...
		{				
			...
			_LOG_INFO_("Send to BackServer,Wait for  BackServer Response. SN[%d]\n",seqNo);
		}
	}
	else
	{
...
int Despatch::doBackResponse(CDLSocketHandler* clientHandler, InputPacket* pPacket)
{
	...
	if(pt->process==NULL &&	pt->client==NULL )
	{
		...
		_LOG_INFO_("==============WorkProcess[%d]:doBackResponse OK===========\n\n",getpid());
		return 0;
	}
