...
void Player::login()
{
	...
	if(strlen(json)>=2 && json[0]=='{')
	{
		...
		{   
			...
			//{
				...
				strncpy(headlink, link.c_str(), sizeof(headlink));
				headlink[sizeof(headlink) - 1] = '\0';
				this->m_nWin = winNum;
	...
	_LOG_INFO_(" == Player::login == uid[%d] money[%ld] tax[%d]\n", this->id, this->m_lMoney, this->tax);
...
{
	_LOG_INFO_(" == leave == uid[%d] money[%ld] tax[%d]\n", this->id, this->m_lMoney, this->tax);
