...
namespace KernelEngine {
    ...
    class CForwardItemMgr;
	class CActiveItemMap;
	...
	*/
	...
	public:
		CActiveItemMap();
		~CActiveItemMap();
		bool addItem(ForwardStruct *pForwardStruct );
		//只是从map中移除，不删除存在的结构体
		bool delItemByClient(TCPNetworkItemClient *pClientItem);
		bool delItemByForward(TCPNetworkItemForward *pForwardItem);
		void CloseSocket();
		void clear();
		ForwardStruct *findItemByClient(TCPNetworkItemClient *pClientItem);
		ForwardStruct *findItemByForward(TCPNetworkItemForward *pForwardItem);
		uint32_t getClientCount() { return _mpClientItem.size(); }
		uint32_t getForwardCount() { return _mpForwardItem.size(); }

		std::queue<ForwardStruct *> checkTimeout(std::mutex&  mutex_);
	private:
		typedef std::map<int64_t, ForwardStruct *> FSMap;
		FSMap _mpClientItem;  // 活动连接
		FSMap _mpForwardItem;  // 活动连接
	};

	class CForwardItemMgr
	{
		friend class TCPNetworkItemClient;
		friend class TCPNetworkItemForward;
	...
	public:
		...
		size_t getNetworkItemCount();
		ForwardStruct *findItembyClient(TCPNetworkItemClient *pClientItem);
		ForwardStruct *findItemByForward(TCPNetworkItemForward *pForwardItem);
		TCPNetworkItem *activeNetworkItem(ITCPNetworkItemSink *pITCPNetworkItemSink);
		ForwardStruct * ClientItemToFree(TCPNetworkItemClient *pClientItem);
		ForwardStruct * ForwardItemToFree(TCPNetworkItemForward *pForwardItem);
		std::queue<ForwardStruct *> checkTimeout();
	private:
		TCPNetworkItem *getFreeNetworkItem();
		TCPNetworkItem *addNetworkItem(ITCPNetworkItemSink *pITCPNetworkItemSink);
		typedef std::vector<ForwardStruct *> FSVec;
	//	typedef std::map<int64_t, ForwardStruct *> FSMap;
		typedef std::queue<ForwardStruct *> FSQue;
		FSVec _vecNetworkItemStorage; // 存储连接
		CActiveItemMap _mapNetworkItemActive;  // 活动连接
		FSQue _queNetworkItemBuffer;  // 空闲连接

		std::mutex  _mutex;           // 条目互斥
	};
...
//		std::recursive_mutex  _mutex;           // 条目互斥
    ...
    public:
        // 设置参数
        bool setServiceParameter(int nServicePort, const std::string &forwardIp, int nForwardPort);
   ...
   //     size_t _nMaxConnect;
		...
		CForwardItemMgr _networkItemMgr;
    ...
    public:
        ...
        bool skipDataBuffer(size_t size) 
		{
			...
			{
				LOGGER(E_LOG_DEBUG) << "skipDataBuffer size > _dataBufferUseSize";
				size = _dataBufferUseSize;
			}


            _dataBufferIndex += size; // 索引后移
            _dataBufferUseSize -= size; // 使用的大小 变少
            return true;
        }; // 跳过数据
    ...
    class TCPNetworkItem
    {
		...
		friend class CForwardItemMgr;
		friend CActiveItemMap;
    public:
