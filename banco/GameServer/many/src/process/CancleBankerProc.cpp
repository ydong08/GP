...
// 	}
	...
	if(!player->isbankerlist)
	{
		...
		sendErrorMsg(clientHandler, cmd, uid, -10,ErrorMsg::getInstance()->getErrMsg(-10),seq);
		return 0;
	}
	...
	if(table->bankeruid == player->id)
	{
		...
		sendErrorMsg(clientHandler, cmd, uid, -28,ErrorMsg::getInstance()->getErrMsg(-28),seq);
