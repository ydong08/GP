...
int GetPlayInfo::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt)  
{
	...
	LOGGER(E_LOG_INFO)<<"==>[GetPlayInfo] ["<<cmd<<"] tid=["<<tid<<"] status=["<<status<<"]";
...
int GetPlayInfo::productBankerRobot(int tid)
{
	...
	if(player)
	{
        ...
        int winnum = 0;
		short roll = 0;
		totalnum = 50000 + rand()%100000;
		winnum = (totalnum*(35 + rand()%26))/100;
		roll += 10 + rand()%700;
		short vip = rand()%100 < 90 ? 1 : 0;
		int randuid = Configure::getInstance()->baseuid + rand()%Configure::getInstance()->randuid;
		std::string nickname;
		std::string headurl;
		int uid = 0;
		int sex = 0;
		...
		data["uid"] = uid;
        data["iconid"] = rand() % 6;
		player->id = uid;//PlayerManager::getInstance()->getRobotUid();
		//if (player->id == -1)
		//	player->id = uid;
		...
		LOGGER(E_LOG_INFO)<<"StartRobot[%d] tid["<<player->id<<"] tid=["<<tid<<"]";
		player->login();
	}
	return 0;
}
...
int GetPlayInfo::productBetRobot(int tid)
{
	...
	if(player)
	{
		...
		if (player->id == -1)
			player->id = uid;
		strcpy(player->json,data.toStyledString().c_str());
		player->money = 300000 + rand() % 50000;
