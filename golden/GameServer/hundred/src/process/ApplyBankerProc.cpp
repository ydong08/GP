...
// 	}
	...
	if(player->m_lMoney < Configure::getInstance().bankerlimit)
	{
		_LOG_ERROR_("Your uid[%d] m_lMoney[%ld] is not enough high[%d]\n", uid, player->m_lMoney, Configure::getInstance().bankerlimit);
...
int ApplyBankerProc::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket, Context* pt)
{
