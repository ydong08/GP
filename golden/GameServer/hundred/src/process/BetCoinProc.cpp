...
// 	}
	...
	if(player->m_lBetArray[0] == 0 && player->m_lMoney < table->m_nLimitCoin)
	{
		...
		sendErrorMsg(hallhandler, cmd, uid, -13,ErrorMsg::getInstance()->getErrMsg(-13),seq);
		return 0;
	}

	int64_t diffmoney = (player->m_lBetArray[0]+player->m_lMoney) - (player->m_lBetArray[0]+betmoney) * MAX_MUL;
	...
	if(banker)
	{
		if((table->m_lTabBetArray[0] + betmoney) * MAX_MUL > banker->m_lMoney)
		{
	...
	{
		_LOG_ERROR_("Your uid[%d] Not Have Banker\n", uid);
		sendErrorMsg(hallhandler, cmd, uid, 1,ErrorMsg::getInstance()->getErrMsg(1),seq);
		return 0;
	}
...
int BetCoinProc::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt)  
{
