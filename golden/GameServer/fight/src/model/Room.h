...
class Room
{
	public:
		...
		Table* getAvailableTable(Player* player);
		Table* getAvailableTable(int table_id);
		Table* getAvailableTable(int uid, std::string strip);
		Table* getTable(int table_id);
		Player* getAvailablePlayer();
		Player* getPlayer(int seat_id);
		Player* getPlayerUID(int uid);
		Table* getChangeTable(int changeTid);

		inline int getMaxTableCount(){return max_table_count;}; 
		inline std::vector<Table*> getAllTables(){ return tables; };
		inline std::vector<Player*> getAllPlayers(){ return players; };
