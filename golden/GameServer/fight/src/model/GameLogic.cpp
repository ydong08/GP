...
void GameLogic::ShuffleCard(BYTE cbCardData[], BYTE cbBufferCount)
{
    BYTE count = 0, positiion = 0;
    do
    {
        positiion=rand()%(cbBufferCount-count);
		++count;
        BYTE card = cbCardData[cbBufferCount - count];
        cbCardData[cbBufferCount - count] = cbCardData[positiion];
		cbCardData[positiion] = card;
     
    } while (count<cbBufferCount);
}
...
void GameLogic::RandCardList(BYTE cbCardBuffer[], BYTE cbBufferCount)
{
    ...
    // 洗牌
    ShuffleCard(cbCardData , sizeof(m_cbCardListData));
	...
	do
	{
		bPosition=(rand()*tvCur.tv_usec)%(CountArray(m_cbCardListData)-bRandCount);
...
BYTE GameLogic::ChangeCard(BYTE cbCardBuffer[], BYTE cbBufferCount, BYTE cbCardValue)
{
	...
	if (!isFound)
	{
		...
		{
			if (m_bAvailCardList[i] > 0)
			{
				RetCardValue = m_bAvailCardList[i];
				break;
			}
		}
	}
