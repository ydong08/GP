...
int BetRaseProc::doRequest(CDLSocketHandler* clientHandler, InputPacket* pPacket,Context* pt)
{
	...
	if( ! player->isActive())
	{
		_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] tm_nStatus=[%d] is not active\n",uid, tid, realTid, table->m_nStatus);
	...
	{
		_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] player->m_lCarryMoney=[%ld] is less bet money[%d] countmoney[%ld]\n",uid, tid, realTid, player->m_lCarryMoney, betmoney, player->betCoinList[0] + betmoney);
	...
	{
		_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] betCountmoney=[%ld] is less table m_lCurrMax[%ld]\n",uid, tid, realTid, betCountmoney, table->m_lCurrMax);
		return sendErrorMsg(hallhandler, cmd, uid, -14,ErrorMsg::getInstance()->getErrMsg(-14),seq);
	}

	if(table->m_bIsOverTimer)
	{
		_LOG_ERROR_("[BetRaseProc] table is IsOverTimer uid=[%d] tid=[%d] realTid=[%d] Table Status=[%d]\n",uid, tid, realTid, table->m_nStatus);
		return sendErrorMsg(hallhandler, cmd, uid, 2,ErrorMsg::getInstance()->getErrMsg(2),seq);
	}
	...
	data["rasemoney"] = (double)betmoney;
	...
	if(betCountmoney > table->m_lCurrMax)
	{
		table->m_lCurrMax = betCountmoney;
	...
	_LOG_INFO_("[BetRaseProc] Uid=[%d] GameID=[%s] RaseMoney=[%ld] BetCountMoney=[%ld] currRound=[%d]\n", player->id, table->getGameID(), betmoney, player->betCoinList[0], table->m_bCurrRound);
	...
	if(nextplayer)
	{
		...
		table->setPlayerOptype(player, OP_RASE);
	}
	table->setSumPool();
	player->setActiveTime(time(NULL));
	...
	for(int i = 0; i < GAME_PLAYER + (int)table->m_Viewers.size(); ++i)
	{
		...
		if(otherplayer)
		{
			sendRaseInfoToPlayers(otherplayer, table, player, betmoney , nextplayer,seq);
		}
	}
	//当前已经没有下一个用户下注了,此轮结束
	if(nextplayer == NULL)
...
int BetRaseProc::sendRaseInfoToPlayers(Player* player, Table* table, Player* raseplayer,int64_t betmoney,Player* nextplayer,short seq)
{
	...
	response.WriteInt64(raseplayer->betCoinList[table->m_bCurrRound]);
	response.WriteInt64(raseplayer->m_lCarryMoney);
...
REGISTER_PROCESS(CLIENT_MSG_BET_RASE, BetRaseProc)