...
int GetPlayInfo::doRequest(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt )
{
	...
	if(room->getStatus()==-1)
	{
		return sendErrorMsg(clientHandler, -1, "room.m_nStatus=-1, Server Wait For Stop") ;
	}
...
int GetPlayInfo::sendErrorMsg(CDLSocketHandler* clientHandler, short errcode, const char* errmsg)
{
	OutputPacket response;
	response.Begin(ADMIN_GET_PLAY_INFO);
