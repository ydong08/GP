...
__ROBOT_LOGIN__:
	...
	if(player==NULL)
	{
		...
		if(player == NULL)
		{
			...
			return sendErrorMsg(hallhandler, cmd, uid, -32,ErrorMsg::getInstance()->getErrMsg(-31),pt->seq);
		}
		player->id = uid;
		player->m_lMoney = m_lMoney;
		player->m_nHallid = hallhandler->hallid;
		strncpy(player->name, name.c_str(), sizeof(player->name));
		player->name[sizeof(player->name) - 1] = '\0';
		strncpy(player->json, json.c_str(), sizeof(player->json));
		player->json[sizeof(player->json) - 1] = '\0';
