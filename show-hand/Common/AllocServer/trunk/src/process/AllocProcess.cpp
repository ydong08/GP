...
int AllocProcess::doRequest(CDLSocketHandler* client, InputPacket* pPacket, Context* pt )
{
	...
	_LOG_INFO_("==>[AllocProcess] [0x%04x] uid[%d]\n", cmd, uid);
...
int AllocProcess::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket,Context* pt)  
{
	...
	_LOG_INFO_("==>[AllocProcess] doResponse [0x%04x]\n", cmd);
	...
	/*if(level == 26)
	{
		...
		ReportPkg.WriteShort(-3);
		ReportPkg.WriteString(ErrorMsg::getInstance()->getErrMsg(2));
		ReportPkg.End();
		_LOG_INFO_("Level[%d] is not open\n", level);
		if(pt->client)
		{
	...
	_LOG_DEBUG_("[DATA Reponse] money=[%ld] \n", money);
	//_LOG_DEBUG_("[DATA Reponse] m_nServerId=[%d] \n", pTable->m_nServerId);
	_LOG_INFO_("[DATA Reponse] AllocProcess doResponse m_nTid=[%d] \n", pTable->m_nTid);
