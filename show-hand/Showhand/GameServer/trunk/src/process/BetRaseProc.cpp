...
BetRaseProc::BetRaseProc()
{
	this->name = "BetRaseProc" ;
}
...
int BetRaseProc::doRequest(CDLSocketHandler* clientHandler, InputPacket* pPacket,Context* pt)
{
	//_NOTUSED(pt);
	...
	_LOG_INFO_("==>[BetRaseProc] [0x%04x] uid[%d]\n", cmd, uid);
	...
	if(table == NULL)
	{
		_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] Table is NULL\n",uid, tid, realTid);
	...
	{
		//_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] player->carrycoin=[%d] is less bet money[%d] countmoney[%d]\n",
		...
		betmoney = player->carrycoin - player->betCoinList[0];
	}

	int64_t betCountmoney = player->betCoinList[table->currRound] + betmoney;
	if(betCountmoney < table->currMaxCoin)
	{
		_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] betCountmoney=[%ld] is less table currMaxCoin[%ld]\n",
	...
	//超过当前桌子下注上限
	if(table->maxbetcoin > 0)
	{
		if(betmoney > player->nextlimitcoin)
		{
			//_LOG_ERROR_("[BetRaseProc] uid=[%d] tid=[%d] realTid=[%d] betmoney=[%ld] is larger table currRound[%d] limit[%ld]\n",
			//uid, tid, realTid, betmoney, table->currRound,player->nextlimitcoin);
			//return sendErrorMsg(hallhandler, cmd, uid, -16,ErrorMsg::getInstance()->getErrMsg(-16),seq);
			betmoney = player->nextlimitcoin;
		}
	}
	//设置当前桌子下注最大，是这个加注的用户
	...
	for(i = 0; i < GAME_PLAYER; ++i)
	{
		...
		{
			sendRaseInfoToPlayers(table->player_array[i], table, player, betmoney , nextplayer,seq);
			sendNum++;
		}
	}
	//当前已经没有下一个用户下注了此轮结束
...
int BetRaseProc::sendRaseInfoToPlayers(Player* player, Table* table, Player* betcallplayer,int64_t betmoney,Player* nextplayer,short seq)
{
	int svid = Configure::getInstance()->server_id;
	int tid = (svid << 16)|table->id;
	OutputPacket response;
	response.Begin(CLIENT_MSG_BET_RASE, player->id);
	...
	response.End();
	_LOG_DEBUG_("<==[BetRaseProc] Push [0x%04x] to uid=[%d]\n", CLIENT_MSG_BET_RASE, player->id);
...
int BetRaseProc::doResponse(CDLSocketHandler* clientHandler, InputPacket* inputPacket ,Context* pt )  
{
