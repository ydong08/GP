...
int Despatch::doFrontRequest(CDLSocketHandler* clientHandler,  InputPacket* pPacket)
{
	...
	_LOG_INFO_( "Recv Front Request, Cmd=[0x%04x] Source[%d] sn[%d]\n" ,nCmdType,Source,sn) ;

	if(nCmdType == ADMIN_MSG_ROOMINFO || nCmdType == ADMIN_GET_FREE_PLAYER || nCmdType == ADMIN_GET_SYS_CFG || 
		nCmdType == ADMIN_RELOAD || nCmdType == ADMIN_GET_ALL_PLAYER)
	{
...
int Despatch::doBackResponse(CDLSocketHandler* clientHandler, InputPacket* pPacket)
{
	...
	_LOG_INFO_( "Recv Back Response, Cmd=[0x%04x] Source[%d] sn[%d]\n" ,nCmdType,Source,sn) ;
	if(sn==0)//
	{
		IProcess* processer = ProcessFactory::getProcesser(nCmdType);
		if(processer)
	...
	_LOG_INFO_( "Find callback process , processer=[%s] cmd=[%d] seq=[%d]\n", process->name.c_str(), nCmdType,  pt->seq);
