...
VerifyRedis1::VerifyRedis1()
{
    heartBeatTimer = new Redis1Timer();
...
int VerifyRedis1::InitRedis()
{
	if(m_redis)
        redisFree(m_redis);
    m_redis = Util::initRedisContext(Options::instance()->verifyredis_ip1, Options::instance()->verifyredis_port1);
    if (NULL == m_redis)
		return -1;
...
int VerifyRedis1::getUserNameSex(int &uid, char *nick, short &sex)
{
	if(m_redis == NULL)
    {
        if(InitRedis() < 0)
...
//=============================Redis1Timer===============================
int Redis1Timer::ProcessOnTimerOut()
{
	//_LOG_DEBUG_("\n");
	//_LOG_DEBUG_( "==============Redis1Timer===========\n" );
	LOGGER(E_LOG_DEBUG)<< "==============Redis1Timer===========";
	if(redissrv)
	{
